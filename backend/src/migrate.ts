...
CREATE TABLE IF NOT EXISTS products (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  product_number bigint GENERATED BY DEFAULT AS IDENTITY UNIQUE,
  ...
  product_type varchar(24) NOT NULL DEFAULT 'coffee' CHECK (product_type IN ('coffee','herbalTea','instantDrink','food','other')),
  roast_type varchar(30) CHECK (roast_type IN ('light','medium','mediumDark','dark')),
  coffee_type varchar(20) CHECK (coffee_type IN ('bean','ground')),
  grind_type varchar(30) CHECK (grind_type IN ('espresso','mokaPot','frenchPress','turkish','filter','none')),
  blend_type varchar(120),
  sort_order smallint NOT NULL DEFAULT 100,
  sale_type varchar(20) NOT NULL DEFAULT 'weighted' CHECK (sale_type IN ('weighted','packaged')),
  available_weights_grams integer[] NOT NULL DEFAULT ARRAY[250,500,1000]::integer[],
  package_weight_grams integer NOT NULL DEFAULT 250 CHECK (package_weight_grams > 0 AND package_weight_grams <= 100000),
...
ALTER TABLE products ADD COLUMN IF NOT EXISTS available_weights_grams integer[] NOT NULL DEFAULT ARRAY[250,500,1000]::integer[];
ALTER TABLE products ADD COLUMN IF NOT EXISTS package_weight_grams integer NOT NULL DEFAULT 250;
UPDATE products SET available_weights_grams = ARRAY[package_weight_grams] WHERE sale_type = 'packaged';
UPDATE products SET available_weights_grams = ARRAY[250,500,1000]
WHERE sale_type = 'weighted' AND (available_weights_grams IS NULL OR cardinality(available_weights_grams) = 0);
ALTER TABLE products DROP CONSTRAINT IF EXISTS products_available_weights_grams_check;
ALTER TABLE products ADD CONSTRAINT products_available_weights_grams_check
  CHECK (cardinality(available_weights_grams) BETWEEN 1 AND 20);
ALTER TABLE products ADD COLUMN IF NOT EXISTS product_number bigint GENERATED BY DEFAULT AS IDENTITY;
CREATE UNIQUE INDEX IF NOT EXISTS products_product_number_unique ON products(product_number);
ALTER TABLE products ADD COLUMN IF NOT EXISTS product_type varchar(24) NOT NULL DEFAULT 'coffee';
ALTER TABLE products DROP CONSTRAINT IF EXISTS products_product_type_check;
ALTER TABLE products ADD CONSTRAINT products_product_type_check CHECK (product_type IN ('coffee','herbalTea','instantDrink','food','other'));
ALTER TABLE products ALTER COLUMN roast_type DROP NOT NULL;
ALTER TABLE products ALTER COLUMN coffee_type DROP NOT NULL;
ALTER TABLE products ALTER COLUMN grind_type DROP NOT NULL;
ALTER TABLE products ALTER COLUMN grind_type DROP DEFAULT;
ALTER TABLE products ALTER COLUMN blend_type DROP NOT NULL;
ALTER TABLE products DROP CONSTRAINT IF EXISTS products_package_weight_grams_check;
ALTER TABLE products ADD CONSTRAINT products_package_weight_grams_check CHECK (package_weight_grams > 0 AND package_weight_grams <= 100000);
UPDATE products p
SET product_type = CASE
  WHEN c.slug = 'herbal-tea' THEN 'herbalTea'
  WHEN c.slug = 'cafe-drinks' THEN 'instantDrink'
  ELSE p.product_type
END
FROM categories c
WHERE p.category_id = c.id AND p.product_type = 'coffee';
