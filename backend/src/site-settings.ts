...
export const updateSiteSettings = async (pool: Pool, input: unknown) => {
  ...
  const result = await pool.query<Record<string, unknown>>(
    `UPDATE site_settings SET
      ...
      search_indexing_enabled=$35,invoice_national_id=$36,
      content_ai_api_key=CASE WHEN $37 <> '' THEN $37 ELSE content_ai_api_key END,
      content_ai_model=$38,
      content_ai_instructions=$39,content_ai_default_audience=$40,content_ai_default_tone=$41,
      content_ai_default_length=$42,content_ai_default_language=$43,
      updated_at=now()
